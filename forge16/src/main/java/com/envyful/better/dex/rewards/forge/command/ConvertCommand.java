...
public class ConvertCommand {
    ...
    @CommandProcessor
    public void runCommand(@Sender ICommandSource sender, String[] args) {
        UtilForgeConcurrency.runSync(() -> {
            File file = Paths.get("config/SimpleDexRewards/data.json").toFile();

            if (!file.exists()) {
                sender.sendMessage(new StringTextComponent("There is no data to convert."), Util.NIL_UUID);
                return;
            }
            ...
            try {
                ...
                try (Connection connection = BetterDexRewards.getInstance().getDatabase().getConnection();
                    ...
                    sender.sendMessage(new StringTextComponent("Added all the users"), Util.NIL_UUID);
